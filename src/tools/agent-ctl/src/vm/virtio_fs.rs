...
// Source: a rw root path created in /tmp and appended with the test vm name
...
pub(crate) async fn setup_virtio_fs(
    ...
    let virtiofsd_pid = start_virtiofsd(shared_fs_info.clone(), &host_path).await.context("virtio-fs:: starting daemon")?;
...
fn virtiofsd_args(cfg: SharedFsInfo, shared_dir: &str, sock_path: &str, disable_guest_selinux: bool) -> Result<Vec<String>> {
...
async fn start_virtiofsd(share_fs_info: SharedFsInfo, root_path: &str) -> Result<u32> {
    ...
    let disable_guest_selinux = true;
    let args = virtiofsd_args(share_fs_info.clone(), root_path, &sock_path, disable_guest_selinux).context("virtiofsd args")?;
...
pub(crate) async fn shutdown_virtiofsd(info: SharedFs) -> Result<()> {
    ...
    std::fs::remove_dir_all(&info.shared_path).context("virtio-fs: Failed to delete shared path")?;
